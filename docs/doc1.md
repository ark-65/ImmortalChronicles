...
- 每岁刷新基础 1 点 AP；若拥有“气运之子”体质 +1，familyScore≥95 再 +1，顶配合计 3 点/年。
- 本游戏采用**全手动交互模式**：
  - 玩家必须手动选择每年想要执行的事件选项（如修炼、探索、社交）。
  - 消耗 1 点 AP 执行选中的手动事件。
  - 当 AP 耗尽或玩家决定不再进行更多行动时，点击“进入下一岁”结束本年。
- “进入下一岁”将触发一个默认的随机年岁事件（如“平淡一年”），该事件不消耗 AP，但会使年龄 +1 并重置下一年 AP。

高参与度耗时与机缘等级

- **系统机缘等级 (Opportunity Tiers)**：与功法等级保持一致，决定事件的稀有度、难度及所需耗时。
  | 等级 | 对应评价 | 预计耗时 (年) | 描述 / 场景示例 |
  | :--- | :---: | :---: | :--- |
  | **道级 (Dao)** | **SSS** | 50 - 100 | 万古机缘、大道留痕、禁忌传承、顶级渡劫 |
  | **圣级 (Sheng)** | **SS** | 20 - 50 | 圣地秘境、大能遗宝、上古战场核心 |
  | **仙级 (Xian)** | **S / A** | 10 - 20 | 仙人洞府、大宗试炼、深海遗迹、名门恩怨 |
  | **灵级 (Ling)** | **B / C** | 1 - 5 | 灵脉探险、江湖仇杀、宗门杂役成长 |
  | **凡级 (Fan)** | **D / E / F** | 0 - 1 | 凡俗奇遇、街头斗殴、私塾蒙学 |

- **年龄推进**：进入耗时事件（如长达 10 年的闭关）后，系统将事件拆分为多个**决策轮次**。
- **手动选择轮次**：
  - 每一轮（代表 1 年或更久）玩家必须手动选择动作（如：加固阵法、吞服丹药、强行突破、稳扎稳打）。
  - 每个动作都有即时反馈，并消耗对应的 AP 或资源。
  - 玩家可随时根据闭关状态决定是否中途停止，或者坚持到设定的年限结束。
- **手动交互方式**：取消所有自动托管按钮。长周期事件通过“连续决策流”展现，每一岁均需玩家点击确认或进行策略选择。
- **收益结算**：最终收益取决于玩家在长周期内的所有手动决策累加，而非自动缩放。
- **当岁事件上限**：单岁最多 1 个主事件 + 若干手动选项分支。非耗时事件默认消耗 1 AP 并 +1 岁。
...
- **跨年截断**：长事件若将超出寿元，系统会发出红色预警。玩家必须在执行前决定是否冒坐化风险强行继续，或选择缩短时长。
...
- 事件数分页：若单页事件数 >500，则自动压缩展示，防止长途跋涉带来的加载卡顿。
...
- **全手动模式**：本游戏不设半自动或托管引擎。每一年的核心行动（如感悟天道、宗门大比、寻找机缘）均通过显式的选项列表交由玩家抉择。
- **强反馈决策**：即便在过渡年岁，玩家也需要决定这一年是“低调修行”还是“四处打听消息”，这些决策将影响下一年高等级事件的出现概率。
...
- 功法与武技等级系统：
  | 级别 | 对应 Rank | 经验需求 (expRequired) | 描述关键词 |
  | :--- | :---: | :---: | :--- |
  | **道级 (Dao)** | **SSS** | 2500 - 3000 | 法则本源、天道真解、禁忌秘典 |
  | **圣级 (Sheng)** | **SS** | 1200 - 2000 | 半步入道、宗门圣法、上古传承 |
  | **仙级 (Xian)** | **S / A** | 600 - 1100 | 超脱凡俗、仙灵之气、大宗秘传 |
  | **灵级 (Ling)** | **B / C** | 280 - 550 | 灵力运行、江湖名门、修真入门 |
  | **凡级 (Fan)** | **D / E / F** | 100 - 250 | 肉身基础、凡俗武学、生活技巧 |
- 功法特性：提供修炼倍率、属性侧重（力量/智力/防御/身法）。
...
- 灵根与属性全集 (Element Hierarchy)：
  | 类别 | 包含元素 | 修行门槛 / 获取途径 |
  | :--- | :--- | :--- |
  | **基础五行** | 金、木、水、火、土 | 初始 roll 点决定，最广泛的基础。 |
  | **进阶变异** | 雷、风、冰、阴、阳、毒 | 特定体质或机缘（如：雷灵根、太阴之体）。 |
  | **特殊属性** | 力、肉、气、神、魂、生、死、瞳 | 侧重肉身或神魂修行，不限灵根。 |
  | **技艺/副职** | 药、精、术、阵、音、血、变 | 与职业相关（炼药、阵法、傀儡、音律）。 |
  | **至高大道** | 时、空、混沌、虚无、运、灭、因果、星、古 | 仅限天级以上机缘或道级功法，涉及法则。 |
- 五行构成：为角色记录 `elementProfile = {金,木,水,火,土}`（总和 100%），可出现单灵根（≥80%）、双灵根、杂灵根；变异与至高大道元素需特定体质或机缘解锁。
...
本系统将宗门与家族势力划分为严密的等级体系，并与其在世界地图上的地理分布深度整合。

#### 1. 等级体系 (Hierarchy)
由高到低依次为：
- **【上界 - 仙/圣/神】**
  - **霸主 (Overlord)**：六界至尊，掌管一个或多个地域。
  - **顶级家族 / 圣地宗门 (Top/Holy)**：等价级别，通常为霸主的直接附庸。
  - **仙族 / 仙宗 (Immortal)**：上界的中坚力量。
- **【下界 - 灵/人/魔】**
  - **一品至九品**：实力由强到弱，逐渐向凡俗过渡。

#### 2. 地理分布规则 (Distribution Rules)
- **霸主唯一性**：同一个区域（Zone）中不允许同时出现两个霸主级势力。
- **势力分布**：
  - **仙域 (Xian)**：容纳霸主、圣地/顶级、仙宗/仙族。
  - **灵域 (Ling)**：以一品、二品势力为主。
  - **人域 (Ren)**：以三品至九品势力为主。
  - **魔域 (Mo)**：分布与其品级对应的魔道势力。

#### 3. 附庸体系 (Vassal System)
- 每个势力可拥有其下级势力的附庸关系：
  - **霸主级**：拥有顶级家族或圣地宗门作为附庸。
  - **顶级/圣地**：拥有若干仙族、仙宗或下界一品势力作为附庸。
  - **附庸加成**：
    - 主家可获得附庸的资源上缴与情报共享。
    - 附庸可获得主家的庇护（减少灭顶危机概率）与高阶功法赏赐。

#### 4. 出生与世界判定联动
- `familyScore` 的高低不仅决定出生世界，还决定了玩家所属势力的初始品级。
- 极高家境可能直接出生在霸主级势力的嫡系中，自带“护道者”与“顶级天资”。
